...
void ProjectMatrix(Matrix* matrix, Matrix* projMatrix) {
	MultiplyMatrix(matrix, projMatrix, true);

	if (matrix->nums[0][3] != 0.0f) {
		matrix->nums[0][0] /= matrix->nums[0][3];
		matrix->nums[0][1] /= matrix->nums[0][3];
		matrix->nums[0][2] /= matrix->nums[0][3];
	}
}
...
void ProjectVertices(Mesh* mesh, Matrix* projMatrix) {
	for (int i = 0; i < mesh->numOfTriangles; i++) {
		for (int j = 0; j < mesh->triangles[0]->numOfVertices; j++) {
			ProjectMatrix(mesh->triangles[i]->vertices[j]->xyz, gProjectionMatrix);
		}
	}
}
...
void ScaleMatrix(Matrix* matrix) {
	matrix->nums[0][0] += 1.0f;
	matrix->nums[0][1] += 1.0f;

	matrix->nums[0][0] *= 0.5f * (float)gScreenWidth;
	matrix->nums[0][1] *= 0.5f * (float)gScreenHeight;
}

void ScaleVertices(Mesh* mesh) {
	for (int i = 0; i < mesh->numOfTriangles; i++) {
		for (int j = 0; j < mesh->triangles[0]->numOfVertices; j++) {
			ScaleMatrix(mesh->triangles[i]->vertices[j]->xyz);
		}
	}
}
...
void DrawMesh(Mesh* mesh, SDL_Renderer* renderer, bool rotate, double elapsedTime) {
	...
	TranslateVertices(copyOfMesh, 15.0f);

	Matrix** normals = malloc(sizeof(Matrix*) * copyOfMesh->numOfTriangles);
	...
	for (int i = 0; i < copyOfMesh->numOfTriangles; i++) {
		normals[i] = Normal(copyOfMesh->triangles[i]);
		...
		if (line == NULL) {
			FreeMatrix(normals[i]);
			break;
		}

		float dotProduct = DotProduct(line, normals[i]);
	...
	Matrix* light = InitMatrix(1, 4);
	light->nums[0][0] = 0.0;
	light->nums[0][1] = 0.0;
	light->nums[0][2] = -1.0;
	light->nums[0][3] = 0.0;

	float normalLight = DotProduct(light, light);
	light->nums[0][0] /= normalLight;
	light->nums[0][1] /= normalLight;
	light->nums[0][2] /= normalLight;
	light->nums[0][3] /= normalLight;

	for (int i = 0; i < copyOfMesh->numOfTriangles; i++) {
		if (copyOfMesh->triangles[i]->draw) {
			float lightingDirection = DotProduct(light, normals[i]);
			float intermediate = (lightingDirection + 1.0) / 2.0;
			int colourStrength = (int)(intermediate * 255.0);
			//printf("\nLighting Direction: %0.5f | Colour Strength: %0.5f | Intermediate: %0.5f", lightingDirection, colourStrength, intermediate);
			...
			SDL_SetRenderDrawColor(renderer, colourStrength, colourStrength, colourStrength, 0xFF);
			...
			// Calculate centriod of triangle to visualse normal
			// Code trying to visualise normals similar to blender
			/*
			Matrix* centriod = InitMatrix(1, 3);

			float currentLength = sqrt(normals[i]->nums[0][0] * normals[i]->nums[0][0] + normals[i]->nums[0][1] * normals[i]->nums[0][1] + normals[i]->nums[0][2] * normals[i]->nums[0][2]);
			if (currentLength == 0) { currentLength = 0.1; }
			float factor = 0.01 / currentLength;
			normals[i]->nums[0][0] *= factor;
			normals[i]->nums[0][1] *= factor;
			normals[i]->nums[0][2] *= factor;

			printf("\nNormal Matrix");
			PrintMatrix(normals[i]);

			ProjectMatrix(normals[i], gProjectionMatrix);
			ScaleMatrix(normals[i]);

			printf("\nProjected Normal Matrix");
			PrintMatrix(normals[i]);
			
			centriod->nums[0][0] = (copyOfMesh->triangles[i]->vertices[0]->xyz->nums[0][0] + copyOfMesh->triangles[i]->vertices[1]->xyz->nums[0][0] + copyOfMesh->triangles[i]->vertices[2]->xyz->nums[0][0]) / 3;
			centriod->nums[0][1] = (copyOfMesh->triangles[i]->vertices[0]->xyz->nums[0][1] + copyOfMesh->triangles[i]->vertices[1]->xyz->nums[0][1] + copyOfMesh->triangles[i]->vertices[2]->xyz->nums[0][1]) / 3;
			centriod->nums[0][2] = (copyOfMesh->triangles[i]->vertices[0]->xyz->nums[0][2] + copyOfMesh->triangles[i]->vertices[1]->xyz->nums[0][2] + copyOfMesh->triangles[i]->vertices[2]->xyz->nums[0][2]) / 3;
			
			SDL_SetRenderDrawColor(renderer, 0x00, 0xFF, 0x00, 0x00);
			SDL_RenderDrawLine(renderer, (int)normals[i]->nums[0][0], (int)normals[i]->nums[0][1], (int)centriod->nums[0][0], (int)centriod->nums[0][1]);
